...
import { InputListSchedules } from '../../../controller/serializers/schedules/inputListSchedules'
import { ListSchedulesOperator } from '../../../controller/operators/schedules/listSchedulesOperator'

export const handler = httpHandler(async (event: APIGatewayProxyEvent, context: Context) => {
  context.callbackWaitsForEmptyEventLoop = false
  const operator = container.get(ListSchedulesOperator)
  const body = event?.queryStringParameters

  console.log({ body })

  const input = new InputListSchedules({
    ...(body as object),
